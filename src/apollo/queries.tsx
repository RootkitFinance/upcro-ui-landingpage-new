...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "cybercorey/empire-cro") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const POSITIONS_BY_BLOCK = (account:any, blocks:any) => {
  let queryString = 'query blocks {'
...
export const PRICES_BY_BLOCK = (tokenAddress:any, blocks:any) => {
  let queryString = 'query blocks {'
  queryString += blocks.map(
    (block:any) => `
...
export const HOURLY_PAIR_RATES = (pairAddress:any, blocks:any) => {
  let queryString = 'query blocks {'
  queryString += blocks.map(
    (block:any) => `
...
export const SHARE_VALUE = (pairAddress:any, blocks:any) => {
  let queryString = 'query blocks {'
  queryString += blocks.map(
    (block:any) => `
      t${block.timestamp}:pair(id:"${pairAddress}", block: { number: ${block.number} }) { 
...
export const MINING_POSITIONS = (account:any) => {
  const queryString = `
    query users {
      user(id: "${account}") {
        miningPosition {
