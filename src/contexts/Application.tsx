...
export function useLatestBlocks() {
  ...
  const [state]:any = useApplicationContext()
...
export function useCurrentCurrency() {
  const [state, { update }]:any = useApplicationContext()
...
export function useTimeframe() {
  const [state, { updateTimeframe }]:any = useApplicationContext()
...
export function useSessionStart() {
  const [state, { updateSessionStart }]:any = useApplicationContext()
...
export function useListedTokens() {
  const [state, { updateSupportedTokens }]:any = useApplicationContext()
  const supportedTokens = state?.[SUPPORTED_TOKENS]

  useEffect(() => {
    async function fetchList() {
      const allFetched = await SUPPORTED_LIST_URLS__NO_ENS.reduce(async (fetchedTokens:any, url:any) => {
        const tokensSoFar = await fetchedTokens
        // const newTokens = await getTokenList(url)
        return Promise.resolve([...tokensSoFar])
      }, Promise.resolve([]))
      let formatted = allFetched?.map((t:any) => t.address.toLowerCase())
