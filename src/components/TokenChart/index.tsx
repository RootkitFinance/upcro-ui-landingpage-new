...
const TokenChart = ({ address, color, base, view } : TokenChartProps) => {
  // settings for the window and candle width
  const [chartFilter, setChartFilter] = useState(view)
  const [frequency, setFrequency] = useState(view == CHART_VIEW.PRICE ? DATA_FREQUENCY.LINE : DATA_FREQUENCY.HOUR)
  ...
  const domain : any = [(dataMin: number) => (dataMin > utcStartTime ? dataMin : utcStartTime), 'dataMax']
  const aspect = below1080 ? 60 / 32 : below600 ? 60 / 42 : 60 / 22

  chartData = chartData?.filter((entry: { date: number }) => entry.date >= utcStartTime)
